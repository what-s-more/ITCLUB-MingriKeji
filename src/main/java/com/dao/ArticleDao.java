...
public class ArticleDao extends SuperDao{
	 ...
	 * @功能： 搜索（10条记录）
	 */
	public List<ArticleSingle> searchArticles(String keyword) throws SQLException{
        String sql="SELECT * FROM tb_article WHERE art_title LIKE ? OR art_content LIKE ?";
        Object[] params={"%"+keyword+"%", "%"+keyword+"%"};
        List<ArticleSingle> articlelist = getList(sql, params);
        return articlelist;
    }
