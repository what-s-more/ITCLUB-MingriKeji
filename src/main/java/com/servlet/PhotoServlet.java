...
public class PhotoServlet extends HttpServlet {
	...
	protected void doListShow(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException {
		...
		if(null!=logoner){
			...
			if(callMaster.getId()==logoner.getId()){
		...
		int masterId=callMaster.getId();
		try {
			callMaster = userDao.getMasterSingle(masterId);
		} catch (SQLException e1) {
			e1.printStackTrace();
		}
		session.setAttribute("callBlogMaster",callMaster);

		String showPage=request.getParameter("showPage");
		String goWhich="photo?action=listShow";
		
		List photoList=null;
		PhotoDao photoDao=new PhotoDao();		
		try {
			photoList=photoDao.getListPhoto(masterId,showPage, goWhich);
	...
	protected void doInsert(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException {
		...
		if("upload".equals(type)){												//单击＂上传图片＂提交按钮触发的请求
			try {
				...
				message="只允许上传 <b>jpg、jpeg、gif</b> 格式图片！";
				e1.printStackTrace();				
